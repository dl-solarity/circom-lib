...
async function testHash160Bits(input1, circuit) {
  ...
  const hashBuffer = crypto.createHash("sha1").update(buffer).digest("hex");
...
async function testHash224Chunks(input1, circuit) {
  ...
  const hashBuffer = crypto.createHash("sha224").update(buffer).digest("hex");
...
async function testHash384Chunks(input1, circuit) {
  ...
  const hashBuffer = crypto.createHash("sha384").update(buffer).digest("hex");
...
async function testHash512Chunks(input1, circuit) {
  ...
  const hashBuffer = crypto.createHash("sha512").update(buffer).digest("hex");
